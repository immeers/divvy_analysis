...
clean_data <- function(divvy){
  ...
  #   group_by(start_station_name) %>% slice_head(n = 1) %>% ungroup()
   ...
   start_stat <- divvy[,c(10,12)] %>% distinct() %>% arrange(desc(start_station_name)) %>% 
    group_by(start_station_name) %>% slice_head(n = 1) %>% ungroup()
   
  # end_stat <- divvy[,c(6,7)] %>% distinct() %>% arrange(desc(end_station_name)) %>% 
  #   group_by(end_station_name) %>% slice_head(n = 1) %>% ungroup()
   
   end_stat <- divvy[,c(11,13)] %>% distinct() %>% arrange(desc(end_station_name)) %>% 
    group_by(end_station_name) %>% slice_head(n = 1) %>% ungroup()
...
#22-23 build model
divvy <- clean_data(divvy)

#make 24 predictions
divvy24 <- clean_data(divvy24)
divvy <- divvy24 %>% na.omit

summary(divvy)
divvy$end_station_id
...
# Edit multipolygon format
neighborhoods_edited <- neighborhoods %>% mutate(the_geom = str_replace_all(the_geom, " ", ","))
neighborhoods_edited <- neighborhoods %>% mutate(the_geom = str_replace_all(the_geom, ",,", ","))


# Create polygon object and convert to sf object
neighborhoods_edited <- data.frame(
  the_geom = c("MULTIPOLYGON(((10 40, 40 40, 40 10, 10 10, 10 40)))",
           "MULTIPOLYGON(((20 30, 30 30, 30 20, 20 20, 20 30)))"),
...
convert_to_polygon <- function(multipolygon_string) {
  ...
  coordinates_matrix <- matrix(as.numeric(unlist(str_split(coordinates, " "))), ncol=2, byrow=TRUE)
 
  st_polygon(list(coordinates_matrix))
}

neighborhoods_edited$geom_sf <- lapply(neighborhoods_edited$the_geom, convert_to_polygon)
neighborhoods_edited$geom_sf <- st_sfc(neighborhoods_edited$geom_sf, crs = 4326)

class(neighborhoods_edited$geom_sf)


#CURRENTLY HERE
  #need to define the point

# Define MultiPolygon with two polygons
multipolygon <- st_multipolygon(list(
  list(st_polygon(list(matrix(c(0, 0, 4, 0, 4, 4, 0, 4, 0, 0), ncol=2, byrow=TRUE)))),
  list(st_polygon(list(matrix(c(5, 5, 9, 5, 9, 9, 5, 9, 5, 5), ncol=2, byrow=TRUE))))
))

# Define the point
point <- st_point(c(3, 3))  # This point should be in the first polygon

# Convert to sf objects
multipolygon_sf <- st_sfc(multipolygon)
point_sf <- st_sfc(point, crs = st_crs(multipolygon_sf))

# Check which polygon contains the point
polygon_index <- sapply(multipolygon_sf, function(polygon) st_contains(polygon, point_sf, sparse = FALSE))

# Identify which polygon contains the point
polygon_found <- which(polygon_index, arr.ind = TRUE)

if (length(polygon_found) > 0) {
  cat("Point is in polygon", polygon_found, "\n")
} else {
  cat("Point is not in any polygon\n")
}


# Define the point
for (row in end_stations){
 
point = st_point(c(avg_end_lng[row], avg_end_lat[row]))
# Convert to sf objects

point_sf <- st_sfc(point, crs = st_crs(neighbourhoods$geom_sf))

# Check which polygon contains the point
polygon_index <- sapply(neighbourhoods$geom_sf, function(polygon) st_contains(polygon, point_sf, sparse = FALSE))

# Identify which polygon contains the point
polygon_found <- which(polygon_index, arr.ind = TRUE)

row$polygon_idx = polygon_found
}
...
top <- station_counts %>% arrange(desc(total))
top <- top[1:ceiling(0.75 * nrow(station_counts)),]
```
...
data <- data %>% group_by(id) %>% mutate(neighbourhood = sample(c(1:5), size =1))
...
data <- data  %>% group_by(neighbourhood) %>% mutate(high_demand_threshold = quantile(in_actual, probs = high_demand_percentile / 100, na.rm = TRUE))
data <- data %>% group_by(neighbourhood) %>% mutate(low_demand_threshold = quantile(in_actual, probs = low_demand_percentile / 100, na.rm = TRUE))


zero <- data[data$in_actual == 0 & data$out_actual == 0,]
...
data <- data  %>% group_by(neighbourhood) %>% mutate(high_supply_threshold = quantile(in_actual, probs = high_supply_percentile / 100, na.rm = TRUE))
                                              
data <- data %>% group_by(neighbourhood) %>% mutate(low_supply_threshold = quantile(in_actual, probs = low_supply_percentile / 100, na.rm = TRUE))
