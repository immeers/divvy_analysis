...
neighborhoods <- neighborhoods %>% mutate(the_geom = str_replace_all(the_geom, " ", ","))
neighborhoods <- neighborhoods %>% mutate(the_geom = str_replace_all(the_geom, ",,", ","))
str_replace()
...
# Function to convert MULTIPOLYGON string to st_polygon object
...
neighborhoods$geom_sf <- lapply(neighborhoods$the_geom, convert_to_polygon)

# Convert the list of polygons to an sf object
neighborhoods$geom_sf <- st_sfc(neighborhoods$geom_sf, crs = 4326)
