...
clean_data <- function(divvy){
  ...
  divvy <- divvy %>% filter(rideable_type != "electric_bike")
  ...
  #   group_by(start_station_name) %>% slice_head(n = 1) %>% ungroup()
   ...
   start_stat <- divvy[,c(5,6)] %>% distinct() %>% arrange(desc(start_station_name)) %>% 
    group_by(start_station_name) %>% slice_head(n = 1) %>% ungroup()
   
  # end_stat <- divvy[,c(6,7)] %>% distinct() %>% arrange(desc(end_station_name)) %>% 
  #   group_by(end_station_name) %>% slice_head(n = 1) %>% ungroup()
   
   end_stat <- divvy[,c(7,8)] %>% distinct() %>% arrange(desc(end_station_name)) %>% 
    group_by(end_station_name) %>% slice_head(n = 1) %>% ungroup()
...
clean_divvy$start_station_name <- paste0(divvy$start_station_id, "-", divvy$start_station_name)
...
```{r}
divvy24[divvy24$start_station_id == "",]
