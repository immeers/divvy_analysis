...
pbp_end <- pbp_end %>%  select(game_date, away_team, season_type, time_of_day) %>% rename(end_time =time_of_day)
pbp_start <- pbp_start %>%  select(game_date,time_of_day)%>% rename(start_time=time_of_day)
dat <- merge(x = pbp_start, y = pbp_end, by = c("game_date"))

head(dat)
  dat$start_time <- as.POSIXct(dat$start_time,format = "%Y-%m-%dT%H:%M:%OSZ", tz = "UTC")
 dat$start_time <- time_at_tz( dat$start_time, tz = "America/Chicago")
 
 
 dat$end_time <- as.POSIXct(dat$end_time,format = "%Y-%m-%dT%H:%M:%OSZ", tz = "UTC")
 dat$end_time <- time_at_tz(dat$end_time, tz = "America/Chicago")
```

```{r}
head(pbp_start)  %>% select(game_date, game_id, time_of_day)
summary(pbp %>% select(time_of_day))
...
wrigley_schedule <- joined_wrigley %>% select(date, Time, local_start_time, local_end_time, season, game_date, game_number, day_night, series_game_number, series_description, teams_away_team_name,venue_name, description, teams_away_score, teams_away_is_winner,teams_home_score,teams_home_is_winner )
...
write.csv(plot2023, file = "member_casual.csv")
...
plot_data3 <- grfield_df %>% select(start_date, gameday, member_casual) %>% group_by(start_date, gameday) %>% mutate(count = n()) %>% unique()
...
plot_data4 <- united_df %>% select(start_date, gameday, member_casual) %>% group_by(start_date, gameday) %>% mutate(count = n()) %>% unique()
...
plot_data2 <- soldier_df %>% select(start_date, gameday, member_casual) %>% group_by(start_date, gameday) %>% mutate(count = n()) %>% unique()
