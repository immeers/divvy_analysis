...
nba <- rbind(nba_20, nba_21, nba_22, nba_23, nba_24)
...
mlb <- mlb %>% filter(venue_name == "Wrigley Field" | venue_name == "Guaranteed Rate Field")
```
...
nfl <- nfl %>%
  filter(gameday >= as.Date('2020-01-01') & gameday <= as.Date('2024-12-31') & home_team == "CHI")
```
...
nhl <- nhl %>% filter(home_team_name == "Chicago Blackhawks")
```
